...
function upsertBridge(
  ...
  const existing = bridges.find(
    item => item.id === id || item.bridgeUrl === normalized,
  );
  ...
  const next: BridgeProfile = {
    ...
    label: label?.trim() || existing?.label || labelFromUrl(normalized),
    lastUsedAt: now,
  };
  return [
    next,
    ...bridges.filter(item => item.id !== id && item.bridgeUrl !== normalized),
  ].slice(0, 24);
}
...
export function useBridgeSettings() {
  ...
  const persistState = useCallback(
    async (nextBridges: BridgeProfile[], nextActiveId: string | null) => {
      const active =
        nextBridges.find(item => item.id === nextActiveId) ??
        nextBridges[0] ??
        null;
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY)
      .then(async raw => {
        ...
        let nextBridges = (parsed.bridges ?? []).filter(
          item => item.bridgeUrl && !isLoopbackBridgeUrl(item.bridgeUrl),
        );
        ...
        await persistState(
          nextBridges,
          parsed.activeBridgeId ?? nextBridges[0]?.id ?? null,
        );
      })
      .catch(err =>
        setError(errorMessage(err, 'failed to load bridge settings')),
      )
      .finally(() => setLoading(false));
  }, [persistState]);

  const persistBridgeUrl = useCallback(
    async (next: string, label?: string) => {
      if (isLoopbackBridgeUrl(next)) {
        throw new Error(
          'Loopback bridge URLs are not supported. Use the LAN URL from the pairing QR or your Caddy domain.',
        );
      }
  ...
  const saveBridgeUrl = useCallback(
    async (label?: string) => {
      try {
        await persistBridgeUrl(draftBridgeUrl, label);
        return true;
      } catch (err) {
  ...
  const setBridgeUrlFromPairing = useCallback(
    async (next: string, label?: string) => {
      await persistBridgeUrl(next, label);
    },
    [persistBridgeUrl],
  );
  ...
  const renameBridge = useCallback(
    async (id: string, label: string) => {
      ...
      if (!trimmed) {
        throw new Error('Computer label cannot be empty.');
      }
      await persistState(
        bridges.map(item =>
  ...
  const removeBridge = useCallback(
    async (id: string) => {
      ...
      await persistState(
        next,
        activeBridgeId === id ? next[0]?.id ?? null : activeBridgeId,
      );
    },
    [activeBridgeId, bridges, persistState],
  );
  ...
  return useMemo(
    () => ({
      ...
      bridges,
      activeBridgeId,
      activeBridge: bridges.find(item => item.id === activeBridgeId) ?? null,
      switchBridge,
      renameBridge,
      removeBridge,
      loading,
